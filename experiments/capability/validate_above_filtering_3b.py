"""
Validate ABOVE-zone filtering with Qwen2.5-3B.
"""
...
def print_summary(results: list[Dict[str, Any]]):
    ...
    print(f"\n{'='*60}")
    print("ABOVE-ZONE FILTERING VALIDATION (Real LLM: Qwen2.5-3B)")
    print(f"{'='*60}")
    print(f"\nTotal tasks: {total}")
...
def main():
    ...
    print("Model: Qwen2.5-3B-Instruct (Q5_K_M)")
    print("=" * 60)
    ...
    solver = LlamaCppSolver(base_url="http://127.0.0.1:8083")
    print("  Connected to llama.cpp server on port 8083")
    ...
    out_path = Path("results/capability_benchmark/above_filtering_validation_3b.json")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            "model": "Qwen2.5-3B-Instruct-Q5_K_M",
            "server": "llama.cpp port 8083 (Vulkan GPU)",
