...
def run_routing_experiment(
    ...
    batch_health = BatchHealthMonitor(window_size=10) if "domain_aware" in protocol else None
    ...
    for task in tasks:
        ...
        elif protocol == "domain_aware_no_revision_triage":
            low_floor = routing_signal_threshold - 0.15
            if domain_drift_detected and not verification.passed:
                decision = "domain_aware_escalate"
                escalation_used = True
                final_correct = True
                final_answer = oracle.solve(task).answer
            elif routing_signal < low_floor and not domain_drift_detected:
                decision = "accept_low_signal"
            elif routing_signal >= escalation_signal_threshold:
                decision = "direct_escalate"
                escalation_used = True
