"""
Validate ABOVE-zone filtering with real LLM (Qwen2.5-0.5B).

Key question: For tasks that are already correct (ABOVE), is feedback wasteful?
For tasks that fail, does feedback retry help (NEAR) or not (BELOW)?

This tests the boundary-local amplification hypothesis on real LLM outputs.
"""
...
def run_above_filtering_validation(tasks: list[BenchmarkTask], solver) -> list[Dict[str, Any]]:
...
def print_summary(results: list[Dict[str, Any]]):
    ...
    print(f"\n{'='*60}")
    print("ABOVE-ZONE FILTERING VALIDATION (Real LLM: Qwen2.5-0.5B)")
    print(f"{'='*60}")
    print(f"\nTotal tasks: {total}")
    ...
    if near:
        ...
        print(f"  No NEAR-zone tasks found — model too weak for partial solutions")
    ...
    if len(near) > 0:
        ...
        print(f"  Inverted-U: [PASS] PRESENT" if len(near) > max(len(above), len(below)) else "  Inverted-U: [WEAK] NEAR not dominant")
    else:
        print(f"  Pattern: ABOVE={len(above)}, NEAR={len(near)}, BELOW={len(below)}")
        print(f"  Inverted-U: [ABSENT] model is either correct or completely wrong")
...
def main():
    ...
    print("  Connected to llama.cpp server on port 8081")

    print("\n[3/3] Running validation (single-shot + feedback retry)...")
    print("  This will take several minutes...")
    results = run_above_filtering_validation(tasks, solver)

    print_summary(results)

    out_path = Path("results/capability_benchmark/above_filtering_validation.json")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            "model": "Qwen2.5-0.5B-Instruct-Q4_K_M",
            "server": "llama.cpp port 8081",
