...
This is not a code implementation ticket. P0 (PredictiveHealthMonitor) has been completed with a negative result: predictive residual is detectable (12.8x domain shift separation) but not superior to BatchHealthMonitor (27.2x) and has no temporal advantage (actually 1.8 tasks later). PredictiveHealthMonitor is demoted to experimental sidecar. The current validated governance signal is BatchHealthMonitor batch-level drift. CEE P1 (SelfState) should not proceed based on predictive residual alone.
...
| `uncertainty_state` | self-confidence, source-confidence, batch_drift_signal (validated), prediction_residual (auxiliary), semantic drift, verifier disagreement |
...
- batch_drift_signal (BatchHealthMonitor — primary validated governance signal, P0 confirmed BatchHealthMonitor > PredictiveHealthMonitor)
- prediction_residual (auxiliary / ablation only — demoted after P0 negative result)
- self-confidence
- source-confidence
- memory relevance
- verifier pressure
...
- batch drift signal (primary governance signal)
- prediction residual (auxiliary)
- self-confidence
- source-confidence
- commitment risk
- memory conflict
...
- self-confidence
- source-confidence
- anomaly score derived from batch drift signal (primary)
- anomaly score derived from predictive residual (auxiliary, post-P0 demotion)
- memory relevance
- future reuse probability
- write risk
- commitment risk
...
### P0: unified-sel PredictiveHealthMonitor — COMPLETED / FAILED

**Status**: P0 has been implemented and validated (2026-04-22). Result: negative.

**Result summary**:
- Predictive residual IS detectable: 12.8x domain shift separation, 2.1x gradual drift separation
- But NOT superior to BatchHealthMonitor: 27.2x domain shift, 4.1x gradual drift
- NO temporal advantage: PredictiveHealthMonitor triggers 1.8 tasks LATER than BatchHealthMonitor
- PredictiveHealthMonitor demoted to experimental sidecar; do not connect to routing or CEE

**Implication**: The primary input to AuthoritySwitchingLaw should be `batch_drift_signal` (BatchHealthMonitor), not `prediction_residual`. Prediction residual may serve as an auxiliary ablation signal in future fused health experiments, but it must not be the primary governance trigger.
...
- ❌ no lead time (1.8 tasks later than static drift)
- ❌ less sensitive (12.8x vs 27.2x separation)
- ✅ zero false positives

### P1: CEE SelfState Schema — BLOCKED (P0 negative, governance signal not yet validated)

**Status**: P0 failed. The condition "only after P0 signal validation" is NOT met. P1 should not proceed until a validated governance signal exists.

If a future signal (e.g., fused BatchHealthMonitor + PredictiveHealthMonitor, or a new candidate) passes validation, P1 can be unblocked.
...
- batch_drift_signal (BatchHealthMonitor — primary, P0 confirmed)
- prediction_residual (auxiliary only — P0 negative)
- self-confidence
- source-confidence
- goal distance
- write risk
...
|---|---|---|---|
| batch drift signal threshold | verify / escalate trigger | drift detection latency, false positive rate | missed detection -> over-alerting |
| predictive residual threshold | auxiliary verify trigger (P0 negative — secondary) | early-warning lead time, false positive rate | missed detection -> over-alerting |
...
1. ~~Finish `unified-sel` P0 `PredictiveHealthMonitor`.~~ **COMPLETED 2026-04-22 — FAILED** (no temporal advantage, lower separation than BatchHealthMonitor).
2. Only after a governance signal validation passes, design CEE `SelfState`. Since P0 failed, the condition is NOT met. Do not proceed to P1 based on predictive residual alone. If a future signal (e.g., fused BatchHealthMonitor + PredictiveHealthMonitor) validates, revisit P1.
3. Only after `SelfState`, implement CEE `WriteLaw`.
4. Only after those, train or fit `AuthoritySwitchingLaw`.

Current actionable direction: keep this document as architecture boundary only. Return to Capability Router main line (monitor_repair_triage, verify_escalate_no_revision, BatchHealthMonitor-based drift latch).
