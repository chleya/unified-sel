...
def run_minimal_validation(seed: int, config: NoBoundaryConfig) -> dict:
    """运行最小验证实验"""
    rng = np.random.default_rng(seed)
...
def main():
    print("=" * 80)
    print("Phase 1：最小验证 - 优化结构池冻结策略")
    print("=" * 80)
    print("\n目标：")
    print("  - 运行 5 个种子的实验")
    print("  - 使用优化后的结构池冻结策略（只冻结剪枝，允许创建新结构）")
    print("  - 目标：avg_acc +0.02（从 0.5055 提升到 0.5255+）")
    print("  - 时间限制：3-4 小时")
    print("=" * 80)
    ...
    for seed in seeds:
        print(f"\n运行 seed {seed}...")
        result = run_minimal_validation(seed, config)
    ...
    output = {
        "experiment": "phase1_minimal_validation",
        "seeds": seeds,
        "results": results,
        "summary": {
    ...
    results_dir = get_results_path("phase1_minimal_validation")
