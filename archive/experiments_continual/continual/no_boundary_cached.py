"""
Unified-SEL no-boundary continual-learning experiment WITH cache + parallel.

这是原 no_boundary.py 的优化版本，添加了：
1. 种子级缓存机制（崩溃不用重跑）
2. 多进程并行优化（6核CPU跑5个并行，效率提升5倍）
"""
...
from core.experiment_utils import (
    run_multiple_seeds_with_cache, 
    config_to_hash_prefix
)
...
def build_summary(runs: List[Dict]) -> Dict:
    return {
        "task_0_accuracy_after_early_stream": summarize_runs(runs, "task_0_accuracy_after_early_stream"),
        "task_0_accuracy_final": summarize_runs(runs, "task_0_accuracy_final"),
        "task_1_final": summarize_runs(runs, "task_1_accuracy_final"),
        "forgetting_task_0": summarize_runs(runs, "forgetting_task_0"),
        "max_structures_seen": summarize_runs(runs, "max_structures_seen"),
        "mean_structures_seen": summarize_runs(runs, "mean_structures_seen"),
        "recent_mean_loss": summarize_runs(runs, "recent_mean_loss"),
    }
...
def run_seed(
    ...
    task_dist_rng = np.random.default_rng(seed)
    ...
    if ewc_lambda > 0.0:
        ...
        ewc = EWCBaseline(in_size=config.in_size, out_size=config.out_size, lr=config.lr, ewc_lambda=ewc_lambda)
        clf.set_ewc_baseline(ewc)

    X_task_0, y_task_0 = make_eval_task(0, n_samples=config.eval_samples_per_task, seed=seed + 1000)
    X_task_1, y_task_1 = make_eval_task(1, n_samples=config.eval_samples_per_task, seed=seed + 2000)
    ...
    for step in range(config.steps):
        progress = min(1.0, step / config.steps)
        x, y = stream_sample(progress, task_dist_rng)
...
def run_experiment_with_window(
    ...
    print(f"[INFO] 使用配置哈希: {cache_prefix}")
    
    def _seed_fn(seed: int) -> Dict:
        return run_seed(seed=seed, config=config, window_size=window_size, ewc_lambda=ewc_lambda)
    
    runs, num_cached = run_multiple_seeds_with_cache(
        seed_fn=_seed_fn,
        seeds=config.seeds,
        experiment_name="continual_no_boundary",
    ...
    print(f"[INFO] 完成: {len(runs)} 个种子，其中 {num_cached} 个来自缓存")
    ...
    output_path = results_dir / f"{timestamp()}_cached.json"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Force rerun all seeds even if cache exists",
    )
