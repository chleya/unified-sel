"""
Phase 2：完整特征 - 实现基于 SEL-Lab 的任务签名特征系统

6 维任务签名特征：
1. conflict_score: 专家输出与基础输出的平均差异
2. conflict_peak: 专家输出与基础输出的最大差异
3. confidence: 输出的动态范围（max - min）
4. input_abs_mean: 输入绝对值的均值
5. input_nonnegative_ratio: 输入非负的比例
6. input_zero_ratio: 输入接近零的比例

目标：
- 实现完整的 6 维任务签名特征
- 使用特征来决定何时使用 snapshot expert 或 current model
...
def compute_task_signature(
    ...
    基于 SEL-Lab phase3_model.py:1220-1224
    """
    # 1. conflict_score: snapshot 和 current 输出的平均差异
    conflict_score = float(np.mean(np.abs(snapshot_probs - current_probs)))
    
    # 2. conflict_peak: snapshot 和 current 输出的最大差异
    conflict_peak = float(np.max(np.abs(snapshot_probs - current_probs)))
    
    # 3. confidence: current 输出的动态范围
    confidence = float(np.max(current_probs) - np.min(current_probs))
    
    # 4. input_abs_mean: 输入绝对值的均值
    input_abs_mean = float(np.mean(np.abs(x)))
    
    # 5. input_nonnegative_ratio: 输入非负的比例
    input_nonnegative_ratio = float(np.mean(x >= 0.0))
    
    # 6. input_zero_ratio: 输入接近零的比例
    input_zero_ratio = float(np.mean(np.abs(x) < 1e-6))
    ...
    return {
        ...
        "input_nonnegative_ratio": input_nonnegative_ratio,
        "input_zero_ratio": input_zero_ratio,
    }


def route_with_task_signature(
    clf: UnifiedSELClassifier,
    x: np.ndarray,
    snapshot: dict,
    signature: dict,
    ...
    基于 SEL-Lab 的 selector 策略：
    - 高 conflict + 低 confidence → 使用 snapshot expert
    - 低 conflict + 高 confidence → 使用 current model
    """
    snapshot_probs = clf._predict_with_snapshot(x, snapshot)
    current_probs = clf.predict_proba(x)
    
    # 简单的路由策略
    # 高 conflict（> 阈值）→ 使用 snapshot expert
    if signature["conflict_score"] > config["conflict_threshold"]:
        return int(np.argmax(snapshot_probs))
    
    # 低 confidence（< 阈值）→ 使用 snapshot expert
    if signature["confidence"] < config["confidence_threshold"]:
        return int(np.argmax(snapshot_probs))
...
def run_full_feature_validation(seed: int, config: NoBoundaryConfig) -> dict:
    """运行完整特征验证实验"""
    rng = np.random.default_rng(seed)
    clf = UnifiedSELClassifier(
    ...
    # 评估完整系统（使用任务签名特征路由）
    route_config = {
        "conflict_threshold": 0.15,  # SEL-Lab 默认值
        "confidence_threshold": 0.22,  # SEL-Lab 默认值
    }
    ...
    for i in range(len(X_task_0)):
        ...
        if clf._snapshot_experts:
            ...
            current_probs = clf.predict_proba(x)
            signature = compute_task_signature(clf, x, snapshot_probs, current_probs)
            pred = route_with_task_signature(clf, x, clf._snapshot_experts[0], signature, route_config)
        else:
            pred = clf.predict(x)
...
def main():
    print("=" * 80)
    print("Phase 2：完整特征 - 实现基于 SEL-Lab 的任务签名特征系统")
    print("=" * 80)
    print("\n6 维任务签名特征：")
    print("  1. conflict_score: 专家输出与基础输出的平均差异")
    print("  2. conflict_peak: 专家输出与基础输出的最大差异")
    print("  3. confidence: 输出的动态范围（max - min）")
    print("  4. input_abs_mean: 输入绝对值的均值")
    print("  5. input_nonnegative_ratio: 输入非负的比例")
    print("  6. input_zero_ratio: 输入接近零的比例")
    print("\n目标：")
    print("  - 实现完整的 6 维任务签名特征")
    print("  - 使用特征来决定何时使用 snapshot expert 或 current model")
    ...
    for seed in seeds:
        print(f"\n运行 seed {seed}...")
        result = run_full_feature_validation(seed, config)
    ...
    output = {
        "experiment": "phase2_full_feature_validation",
        "seeds": seeds,
        "results": results,
        "summary": {
    ...
    results_dir = get_results_path("phase2_full_feature_validation")
